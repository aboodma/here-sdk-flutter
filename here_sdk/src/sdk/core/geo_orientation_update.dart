...
/// For example, if you want bearing not to be updated set it to `null`.
@immutable
class GeoOrientationUpdate {
  /// Bearing in degrees. 0 is north up, positive is clockwise.
  /// `null` value means that bearing is not updated.
  final double? bearing;

  /// Tilt in degrees. 0 is perpendicular to earth surface, positive turns nose up.
  /// `null` value means that tilt is not updated.
  final double? tilt;

  const GeoOrientationUpdate._(this.bearing, this.tilt);

  /// [bearing] Bearing in degrees. When the passed value is `null` bearing is not updated.
  /// NaN value is converted to `null`.
  ///
  /// [tilt] Tilt in degrees. When the passed value is `null` tilt is not updated.
  /// NaN value is converted to `null`.
  ///
